 ...
 * <p>
 * 给GridLayoutManager或StaggeredGridLayoutManager(均为水平方向) 增加透明分割线(与RecyclerView的背景色一致)
 */

public class GridItemDividerTransparentVertical extends RecyclerView.ItemDecoration {
    ...
    /**
     * @param spacing item 间距 如间距16(即代表16dp)
     */
    public GridItemDividerTransparentVertical(Context context, int spacing) {
     ...
     */
    public GridItemDividerTransparentVertical(Context context, int spacing, boolean includeEdge) {
        ...
        if (mStartFromSize <= position && position <= lastPosition - mEndFromSize) {
            ...
            if (mIncludeEdge) {
                ...
                if (spanGroupIndex > -1) {
                    ...
                    boolean isFirstLineStagger = (fullPosition == -1 || position < fullPosition)
                            && (position < mSpanCount);
                ...
                if (spanGroupIndex > -1) {
                    ...
                    // Stagger上间距显示规则
                    boolean isStaggerShowTop = position >= mSpanCount
                            || (fullSpan && position != 0)
                            || (fullPosition != -1 && position != 0);
     ...
     */
    public GridItemDividerTransparentVertical setStartFrom(int startFromSize) {
        this.mStartFromSize = startFromSize;
        return this;
    }

    /**
     * 设置从哪个位置 结束设置间距。如设置为1，表示用户设置了上拉加载。默认为0
     *
     * @param endFromSize 一般为FooterView的个数 + 加载更多布局(不一定设置)
     */
    public GridItemDividerTransparentVertical setEndFromSize(int endFromSize) {
     ...
     *
     * @param startFromSize 一般为HeaderView的个数 + 刷新布局(不一定设置)
     * @param endFromSize   代表为FooterView的个数 + 加载更多布局(不一定设置)，默认为0
     */
    public GridItemDividerTransparentVertical setNoShowSpace(int startFromSize, int endFromSize) {
        this.mStartFromSize = startFromSize;
        this.mEndFromSize = endFromSize;
        return this;
    }
